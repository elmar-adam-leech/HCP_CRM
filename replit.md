...
- Public marketing routes (`/`, `/login`, `/licenses`, `/privacy`, `/terms`, `/signup`, `/forgot-password`, `/reset-password`, `/book/:slug`) tolerate a trailing slash. The pre-mount path check in `App.tsx` normalizes the slash before deciding which shell to mount, and `useStripTrailingSlash` (used inside `PublicShell` and `PublicBookingShell`) does a `replace` navigation so wouter's exact-match `<Switch>` resolves the route.
...
- After any latency-affecting change, capture before/after p50/p95 from `/api/_admin/perf/latency` (super_admin only) for the affected routes and include the numbers in the PR description. Slow individual requests (> `SLOW_REQUEST_THRESHOLD_MS`, default 500ms) are logged with `[slow]` plus method, route, status, duration, and contractorId. Top slow SQL statements are exposed at `/api/_admin/perf/slow-queries` — requires `pg_stat_statements`, which is **not** auto-installed. Install by hand in dev with `CREATE EXTENSION pg_stat_statements;` if you want data; the endpoint returns a clean 503 otherwise. Production never has it.
- JWT validation results are cached in-process for `AUTH_CACHE_TTL_MS` (default 30s, configurable via env). On a hit, the auth middleware skips both the `revoked_tokens` lookup and the `users.tokenVersion` check, eliminating two DB round-trips per authenticated request. Revocation actively invalidates: `AuthService.revokeToken` calls `evictAuthCache(jti)` and the slow-path tokenVersion-mismatch branch evicts defensively. The TTL is the worst-case propagation window for sign-out-all-devices on tokens that haven't yet hit a given replica; restarting the server flushes the cache instantly. See `server/services/auth-cache.ts`.
- Hot read endpoints (`/api/auth/me`, `/api/user/contractors`) hit an in-process cache (`server/services/cache.ts`) with a 60s TTL via `getUserSupplementalCached`, `getEnabledIntegrationsCached`, `getUserContractorsWithDetailsCached`, and the existing `getContractorCached` / `getUserContractorCached`. Mutations to user fields (gmail connect/disconnect, dialpad number, call preference, role/permission changes, contractor add/switch) and contractor fields (integration enable/disable) MUST call `invalidateUserCache(userId)` / `invalidateContractorCache(contractorId)` (or the matching `cacheInvalidation.*` helper) — staleness is otherwise capped at 60s.
- Content-hashed `/assets/*` bundles are served with `Cache-Control: public, max-age=31536000, immutable`. The `no-store` / `max-age=300` middleware in `server/routes.ts` MUST skip `/assets/*` so the long-cache wins (send.js only sets Cache-Control if not already set; pre-setting silently neuters the immutable cache).
- The open-source licenses page (`/licenses`) is fed by `client/public/third-party-licenses.json`, regenerated on every `npm run build` from `scripts/generate-licenses.cjs`. Do not edit the JSON by hand. Adding a new runtime dependency requires no extra step — the next build picks it up.
- Auth-cache hit/miss counters are surfaced under `authCache` (with `hits`, `misses`, `hitRate`, `size`, `ttlMs`, `maxEntries`) on the `/api/_admin/perf/latency` response so we can verify the cache is doing its job. Baseline (30s TTL, before any tuning): hit rate measured at ~96-98% on a typical authenticated dashboard session — well above the 95% target, so raising TTL to 60-120s would yield only marginal additional savings. Re-check this before changing `AUTH_CACHE_TTL_MS`.
...
- Never auto-install Postgres extensions from app code (e.g. `CREATE EXTENSION` in `server/schema-drift.ts`). Replit's deploy pipeline introspects the dev DB and pushes any extension-created system views to prod as plain SQL migrations, which then fail because the underlying functions don't exist there. See task #620 for the deploy block this caused.
...
- **Housecall Pro API**: For bi-directional sync of contacts, estimates, and jobs, and scheduling.
    ...
    - HCP silently ignores `address_id` PATCH on already-created estimates — re-pin must happen at the customer-address layer (or via `PATCH /leads/:id { address_id }` *before* `convertLead`), not on the estimate after the fact. Same quirk has been observed on per-customer-address PATCH and on customer-address DELETE for some legacy rows; the verify-and-recover pattern in `server/scheduling/hcp-customer.ts` handles both.
